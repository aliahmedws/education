...
public class EfCoreLateFeePolicy
    : EfCoreRepository<EHubDbContext, LateFeePolicy, Guid>,
      ILateFeePolicyRepository
{
    public EfCoreLateFeePolicy(IDbContextProvider<EHubDbContext> dbContextProvider)
        ...
        Term? term)
    {
        ...
        var policies = await dbSet
            .Where(x => x.IsActive)
            .Where(x =>
                (!x.GradeLevel.HasValue || x.GradeLevel == gradeLevel) &&
                (!x.Section.HasValue || x.Section == section) &&
                (!x.Shift.HasValue || x.Shift == shift) &&
                (!x.Term.HasValue || x.Term == term))
            .ToListAsync();
